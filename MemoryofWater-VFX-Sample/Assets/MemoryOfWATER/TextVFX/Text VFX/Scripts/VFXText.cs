...
public class VFXText : MonoBehaviour
{
    ...
    private Shape BuildLines(int character)
    {
        ...
        if(Font.Outlines.ContainsKey(character))
        {
    ...
    private Shape BuildWord(string word)
    {
        ...
        {
            Shape newShape = BuildLines(character);
    ...
    {
        Shape shape = BuildWord(word);
        shape = AlignShapeCenter(shape); // make the characters align according to their center
        Color[] colorArray = BuildColorArray(shape);
        Texture2D texture = new Texture2D(colorArray.Length / 2, 2, TextureFormat.RGBAFloat, false);
